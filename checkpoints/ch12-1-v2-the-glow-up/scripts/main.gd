...
var _sfx_zing := AudioStreamPlayer.new()
...
func _ready() -> void:
	...
	camera.make_current()
	_best_at_start = GameState.high_score
	# The world: a pipe with a history of poor maintenance. Muted on
	# purpose — readability is the acceptance test (ch. 12).
	var bg := Sprite2D.new()
	...
	add_child(_music)
	_music.play()
	for pair in [[_sfx_pop, "pop"], [_sfx_whoosh, "whoosh"], [_sfx_sting, "sting"], [_sfx_zing, "zing"]]:
...
func _end_run() -> void:
	...
	art.texture = load("res://assets/sprites/clogged.png")
	art.position = Vector2(270, 330)
	art.scale = Vector2.ONE * (420.0 / float(art.texture.get_width()))
	art.z_index = 50
	add_child(art)
	var beat_the_best := GameState.score > _best_at_start and _best_at_start > 0
	if beat_the_best:
		_sfx_zing.play()
	game_over_label.text = "%sScore: %d   Best: %d\n\npress any key to re-flush\nM for menu" % [
		"NEW BEST!\n" if beat_the_best else "",
